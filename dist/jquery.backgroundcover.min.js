 ...
 *  Under MIT License
 */
!function(a){function b(b,e){this.$element=a(b),this._defaults=d,this._name=c,this.settings=a.extend({},d,this.$element.data(),e),this.setOptions(this.settings),this.init()}var c="backgroundcover",d={image:null,safearea:"0%,0%,100%,100%",resizeInterval:250};b.prototype={init:function(){var a=this;setInterval(function(){a._checkResize()},this.settings.resizeInterval)},setOptions:function(b){this.settings=a.extend({},d,this.$element.data(),b),"undefined"!=typeof b.image?this.setImage(b.image):b.safearea&&this.setSafearea(b.safearea)},_checkResize:function(){var a=this.$element.width(),b=this.$element.height();(a!==this.elementLastWidth||b!==this.elementLastHeight)&&this.layout()},setImage:function(b){function c(a){h.imageWidth=a.width,h.imageHeight=a.height,h.loaded=!0,h.setSafearea(h.settings.safearea)}var d,e,f,g,h=this;if(this.image=b,!this.image){if(d=this.$element.css("background-image"),!d)throw new Error("No background image available");if(e=d.match(/url\([\"\']?([^\"\']*)[\"\']?\)/i),!e)throw new Error("Background image could not be retrieved from CSS property");this.image=e[1]}"undefined"==typeof this.$element.css("background-size")?(this.mode="img",this.$img=a('<img src="'+this.image+'" />').css("position","absolute"),"static"===this.$element.css("position")&&this.$element.css("position","relative"),this.$element.css("background-image","none").css("overflow","hidden").prepend(this.$img)):(this.mode="css",this.$element.css("background-image","url("+this.image+")")),this.loaded=!1,f=new Image,f.onload=function(){c(this)},g=this.image,navigator.userAgent.match(/msie/i)&&(g+="?"+(new Date).valueOf()),f.src=g,f.complete&&c(f)},setSafearea:function(a){if(a=a||d.safearea,this.settings.safearea=a,this.loaded){var b=a.split(","),c=function(a,b){if("number"==typeof a)return a;if("function"==typeof a.trim&&(a=a.trim()),"%"===a.charAt(a.length-1)){var c=parseFloat(a);return Math.round(c/100*b)}return Math.round(parseFloat(a))};this.safearea={x1:c(b[0],this.imageWidth),y1:c(b[1],this.imageHeight),x2:c(b[2],this.imageWidth),y2:c(b[3],this.imageHeight)},this.layout()}},layout:function(){if(this.loaded){var a,b,c,d,e,f,g,h,i,j=this.$element.width(),k=this.$element.height(),l=j/k,m=this.safearea.x1,n=this.safearea.y1,o=this.safearea.x2,p=this.safearea.y2,q=o-m,r=p-n,s=this.imageWidth,t=this.imageHeight,u=s/t,v=q/s,w=r/t;l>u?(a=j,b=Math.round(a/u),i=a/s,r*i>k&&(b=Math.round(k/w),a=Math.round(b*u),i=a/s)):(b=k,a=Math.round(b*u),i=a/s,q*i>j&&(a=Math.round(j/v),b=Math.round(a/u),i=a/s)),d=(b-k)/2,c=(a-j)/2,f=n*i-d,h=b-p*i-d,0>f?d+=f:0>h&&(d-=h),e=m*i-c,g=a-o*i-c,0>e?c+=e:0>g&&(c-=g),c=Math.round(-c),d=Math.round(-d),this.elementLastWidth=j,this.elementLastHeight=k,this._update(a,b,c,d)}},_update:function(a,b,c,d){"css"===this.mode?this.$element.css("background-position",c+"px "+d+"px").css("background-size",a+"px "+b+"px"):"img"===this.mode&&this.$img.css("width",a+"px").css("height",b+"px").css("top",d+"px").css("left",c+"px")}},a.fn[c]=function(d){return this.each(function(){a.data(this,c)?a.data(this,c).setOptions(d):a.data(this,c,new b(this,d))})}}(jQuery,window,document);